...
const sampleCoreTeamData = [
  {
    ...
    bio: '技術教育專家，致力於推廣現代 Web 開發技術與最佳實踐。擁有豐富的前端框架經驗，特別專精於 React 和 Vue.js 開發。',
    profile_image: '/assets/members/yen_profile.jpg',
    sort_order: 1
  }
];
...
function generateCoreSQL() {
  ...
  sampleCoreTeamData.forEach(member => {
    sql += `INSERT INTO core_team (name, title, department, bio, profile_image, sort_order, created_at, updated_at) VALUES\n`;
    sql += `('${member.name}', '${member.title}', '${member.department}', '${member.bio}', '${member.profile_image}', ${member.sort_order}, NOW(), NOW());\n\n`;
  });
  ...
  sql += `INSERT INTO core_team_categories (core_team_id, category_id, created_at, updated_at) VALUES\n`;
